...
import type { StyledLine } from "../pty/ansi"
import { appendToBuffer } from "../pty/ansi"

export type PaneStatus = "idle" | "running" | "waiting" | "error"
...
interface PaneState {
  ...
  appendLines: (id: string, newLines: StyledLine[]) => void
  setStatus: (id: string, status: PaneStatus) => void
  ...
  appendLines: (id, newLines) =>
    set((s) => {
      ...
      return {
        panes: {
          ...s.panes,
          [id]: { ...pane, lines: appendToBuffer(pane.lines, newLines) },
        },
      }
    }),
